...
struct MailView: View {
    ...
    private var message, subject: String?
    private var recipients, ccRecipients, bccRecipients: [String]?
    private var alerts: MailAlertsOptions = .enabled
    ...
    class Coordinator: NSObject, MFMailComposeViewControllerDelegate {
        ...
        func mailComposeController(_ controller: MFMailComposeViewController, didFinishWith result: MFMailComposeResult, error: Error?) {
            guard error == nil else {
                parent.result = MailResult(.failure(error!))
                return
            }
            
            parent.result = MailResult(.success(result))
        }
    }
}

#endif // canImport(MessageUI)
